...
class LlamaFlashAttention(LlamaAttention):
    ...
    def initialize_quant_fa_awq(self):
        ...
        self.k_shape = (self.k_proj.in_features, self.k_proj.out_features)
        self.v_shape = (self.v_proj.in_features, self.v_proj.out_features)
        self.qkv_proj = WQLinear(
            self.q_proj.w_bit,
            self.q_proj.group_size,
            self.q_proj.in_features,
            self.q_proj.out_features + self.k_proj.out_features + self.v_proj.out_features,
            False,
            self.q_proj.qweight.device)
        self.qkv_proj.qweight = torch.empty(self.q_proj.qweight.shape[0] + self.k_proj.qweight.shape[0] + self.v_proj.qweight.shape[0], self.q_proj.qweight.shape[1], dtype=torch.int8,     device=self.q_proj.qweight.device)
        self.qkv_proj.qzeros  = torch.empty(self.q_proj.qzeros.shape[0] + self.k_proj.qzeros.shape[0] + self.v_proj.qzeros.shape[0],  self.q_proj.qzeros.shape[1],  dtype=torch.int8,     device=self.q_proj.qzeros.device)
        self.qkv_proj.scales  = torch.empty(self.q_proj.scales.shape[0] + self.k_proj.scales.shape[0] + self.v_proj.scales.shape[0],  self.q_proj.scales.shape[1],  dtype=torch.bfloat16, device=self.q_proj.scales.device)

        self.qkv_proj.qweight[:self.q_proj.out_features,                            :].copy_(self.q_proj.qweight)
        self.qkv_proj.qweight[ self.q_proj.out_features:self.q_proj.out_features + self.k_proj.out_features, :].copy_(self.k_proj.qweight)
        self.qkv_proj.qweight[ self.q_proj.out_features + self.k_proj.out_features:,                         :].copy_(self.v_proj.qweight)
        self.qkv_proj.qzeros[ :self.q_proj.out_features,                            :].copy_(self.q_proj.qzeros)
        self.qkv_proj.qzeros[  self.q_proj.out_features:self.q_proj.out_features + self.k_proj.out_features, :].copy_(self.k_proj.qzeros)
        self.qkv_proj.qzeros[  self.q_proj.out_features + self.k_proj.out_features:,                         :].copy_(self.v_proj.qzeros)
        self.qkv_proj.scales[ :self.q_proj.out_features,                            :].copy_(self.q_proj.scales)
        self.qkv_proj.scales[  self.q_proj.out_features:self.q_proj.out_features + self.k_proj.out_features, :].copy_(self.k_proj.scales)
        self.qkv_proj.scales[  self.q_proj.out_features + self.k_proj.out_features:,                         :].copy_(self.v_proj.scales)
        if self.q_proj.bias:
            self.qkv_proj.bias = torch.empty((self.q_proj.bias.shape[0] + self.k_proj.bias.shape[0] + self.v_proj.bias.shape[0],), dtype=torch.float16, device=self.q_proj.bias.device)
            self.qkv_proj.bias[:self.q_proj.out_features,                          ].copy_(self.q_proj.bias)
            self.qkv_proj.bias[ self.q_proj.out_features:self.q_proj.out_features + self.k_proj.out_features].copy_(self.k_proj.bias)
            self.qkv_proj.bias[ self.q_proj.out_features + self.k_proj.out_features,                       ].copy_(self.v_proj.bias)

        self.qkv_states_fp   = np.zeros((1, 1, self.q_shape[1] + self.k_shape[1] + self.v_shape[1]), dtype=np.float32)
        ...
        self.key_states_np   = np.empty((1, 1, self.k_shape[1]), dtype=np.float32)
        self.value_states_np = np.empty((1, 1, self.v_shape[1]), dtype=np.float32)
    ...
    def initialize_quant_fa_qlinear(self):
        ...
        self.qkv_proj = torch.empty(self.q_proj.in_features, self.q_proj.out_features + self.k_proj.out_features + self.v_proj.out_features)
        self.qkv_proj[:, :self.q_proj.out_features]                                                     = torch.int_repr(self.q_proj_wb[0]).transpose(0, 1)
        self.qkv_proj[:,  self.q_proj.out_features:self.q_proj.out_features + self.k_proj.out_features] = torch.int_repr(self.k_proj_wb[0]).transpose(0, 1)
        self.qkv_proj[:,  self.q_proj.out_features + self.k_proj.out_features:]                         = torch.int_repr(self.v_proj_wb[0]).transpose(0, 1)
        ...
        self.states_fp = np.zeros((1,1,self.q_shape[1] + self.k_shape[1] + self.v_shape[1]), dtype=np.float32)
        self.states_token = np.zeros((1,self.q_shape[1] + self.k_shape[1] + self.v_shape[1]), dtype=np.int32)
    ...
    def _init_tiling_factors(self, lh, lq, lk):
        ...
        if (lh == -1 or lq == -1 or lk == -1): # Tile factors not provided
            if self.flash_config is None or key not in self.flash_config.keys():
                ...
                # print("!!!!!1: ", self.lh, self.lq, self.lk, self.lq_remainder, self.lk_remainder)
            else:
                self.lh, self.lq, self.lk = self.flash_config[key]
                self.lq_remainder, self.lk_remainder = self.lq, self.lk
                # print("!!!!!2: ", self.lh, self.lq, self.lk, self.lq_remainder, self.lk_remainder)
        else: # Use external factors
            self.lh, self.lq, self.lk = lh, lq, lk
            self.lq_remainder, self.lk_remainder = self.lq, self.lk
            # print("!!!!!3: ", self.lh, self.lq, self.lk, self.lq_remainder, self.lk_remainder)
    ...
    def _qkv_projections_merged(self, hidden_states):
        ...
        if self.is_prefill:
            if self.quant_mode == "awq":
                ...
                states = torch.zeros((1, hidden_states.shape[0], self.q_shape[1] + self.k_shape[1] + self.v_shape[1])) # TODO: B > 1
            ...
            query_states *= self.scaling
            key_states    = states[:, :, self.q_shape[1]:self.q_shape[1] + self.k_shape[1]]
            value_states  = states[:, :, self.q_shape[1] + self.k_shape[1]:]
            ...
            if self.quant_mode == "awq":
                ...
                self.key_states_np = self.qkv_states_fp[:, :, self.q_shape[1]:self.q_shape[1] + self.k_shape[1]]
                self.value_states_np = self.qkv_states_fp[:, :, self.q_shape[1] + self.k_shape[1]:]
                ...
                self.key_states_np = self.states_fp[:, :, self.q_shape[1]:self.q_shape[1] + self.k_shape[1]] * self.k_proj_scale
                if self.k_proj_bias_np is not None:
                    self.key_states_np += self.k_proj_bias_np
                
                self.value_states_np = self.states_fp[:, :, self.q_shape[1] + self.k_shape[1]:] * self.v_proj_scale
    ...
    def _qkv_projections_merged_token_phase(self, hidden_states):
        ...
        key_states = self.qkv_states_torch[:, :, self.q_shape[1]:self.q_shape[1] + self.k_shape[1]]
        value_states = self.qkv_states_torch[:, :, self.q_shape[1] + self.k_shape[1]:]
    ...
    def _forward_llama_attn_prefill(
        ...
        aieout = np.zeros((1, q_len, self.q_shape[1] + self.k_shape[1] + self.v_shape[1]), dtype=np.float32)
    ...
    def _forward_llama_attn(
        ...
        kv_seq_len = key_states.shape[-2]
        # print("past_key_value is None? ", past_key_value[0].shape if past_key_value is not None else None)
        if past_key_value is not None:
            # print("SEQ Shape: ", kv_seq_len, past_key_value[0].shape)
            kv_seq_len += past_key_value[0].shape[-2]
  
        cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)
        # print("emb: ", self.rotary_emb)
